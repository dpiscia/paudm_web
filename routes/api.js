...
job = query_post(req.params.id,req.params.all).then(function(val) {
...
function recursive_query(id,level) {
	return ("with recursive t(level,nbr, super_id,id, task, status, config, input, output , ts_created, ts_started, ts_ended) as ( \
